...
kills stale processes and frees the port on startup, and handles the
...
| `TELEGRAM_ALLOWED_USERS` | Comma-separated whitelist of Telegram user IDs; empty = open to all |
...
| `TELEGRAM_ALLOW_ALL` | README | Set to `true` to explicitly open bot to all users |
